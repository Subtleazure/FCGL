...
class Jacobi(nn.Module):
    def __init__(self, input_dim, hid_dim, output_dim, dropout, K, a=1.0, b=1.0):
        ...
        self.act = nn.ReLU()
        # 【修改 1】: 定义一个可学习的注意力向量
        # 形状 [1, hid_dim]: 所有阶共享同一个评判标准 (或者 [K+1, hid_dim] 让每阶有独立标准)
        # 这里建议先用共享的，因为 W_weight 已经把特征变换到不同空间了
        self.att_vector = nn.Parameter(torch.Tensor(1, hid_dim, 1))
    ...
    def forward(self, data):
        ...
        raw_scores = torch.matmul(
            Hs_stack, self.att_vector).squeeze(-1).t()  # [N, K+1]

        # 1. 移除 LeakyReLU，直接用线性分数，或者用 Tanh 限制幅度 (防止数值爆炸)
        # 建议先试试 Tanh，虽然之前说它导致平滑，但配合 Dropout 使用是安全的
        scores = torch.tanh(raw_scores)

        alpha = F.softmax(scores, dim=1)

        # alpha 全1
        if not args.alpha_at:
            alpha = torch.ones_like(alpha) / (self.K + 1)  # 不是自学习

        if self.training:
            # 固定保留 80% 的通道
            num_keep = int(0.8 * (self.K + 1))
            num_keep = max(1, num_keep)  # 至少保留1个

            # 随机选择要保留的通道索引
            keep_indices = torch.randperm(
                self.K + 1, device=data.x.device)[:num_keep]

            # 创建 mask
            channel_mask = torch.zeros(1, self.K + 1, device=data.x.device)
            channel_mask[0, keep_indices] = 1

            # 广播到 alpha
            alpha_masked = alpha * channel_mask

            # 重新归一化 (让剩下的 alpha 和为 1)
            alpha = alpha_masked / \
                (alpha_masked.sum(dim=1, keepdim=True) + 1e-8)
        # -----------------------------------------------------------
