...
def load_fcgl_dataset(root, dataset="Cora", num_clients=3, classes_per_task=2, shuffle_task=False):
    ...
    if shuffle_task:
        task_dir = os.path.join(
            data.processed_dir, f"task_{classes_per_task}_shuffle")
    else:
        task_dir = os.path.join(
            data.processed_dir, f"task_{classes_per_task}_no_shuffle")
    if not os.path.exists(task_dir):
        os.makedirs(task_dir)

    if dataset in ["Cora", "CiteSeer", "Computers", "Physics", "Actor", "PubMed"]:
    ...
    for client_id in range(num_clients):
        ...
        if not os.path.exists(task_file_path):
            task_file = class_incremental(
                local_data, classes_per_task, train_, val_, test_, shuffle_task=shuffle_task)
        ...
        for task_i in task_file:
            ...
            client_i_task_i_known_classes = torch.unique(
                local_data.y[client_i_task_i_mask])
            known_class_list.append(client_i_task_i_known_classes)

        print(
            f"client {client_id} has {len(processed_data[client_id]['task'])} tasks.")
    ...
    if num_remained_classes != data.global_data.num_global_classes:
        print(
            f"ATTENTION!!! FCGL DROPS {data.global_data.num_global_classes - num_remained_classes} CLASS(ES).")
