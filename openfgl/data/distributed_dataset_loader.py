...
class FGLDataset(Dataset):
    ...
    def check_args(self, args):
        ...
        if args.simulation_mode == "graph_fl_cross_domain":
            assert len(
                args.dataset) == args.num_clients, f"For graph-fl cross domain simulation, the number of clients must be equal to the number of used datasets (args.num_clients={args.num_clients}; used_datasets: {args.dataset})."
        elif args.simulation_mode == "graph_fl_label_skew":
            assert len(
                args.dataset) == 1, f"For graph-fl label skew simulation, only single dataset is supported."
        elif args.simulation_mode == "subgraph_fl_label_skew":
            assert len(
                args.dataset) == 1, f"For subgraph-fl label skew simulation, only single dataset is supported."
        elif args.simulation_mode == "subgraph_fl_louvain_plus":
            assert len(
                args.dataset) == 1, f"For subgraph-fl louvain clustering simulation, only single dataset is supported."
        elif args.simulation_mode == "subgraph_fl_metis_plus":
            assert len(
                args.dataset) == 1, f"For subgraph-fl metis clustering simulation, only single dataset is supported."
    ...
    def get_client_data(self, client_id):
        ...
        data = torch.load(osp.join(self.processed_dir,
                          "data_{}.pt".format(client_id)), weights_only=False)
        if hasattr(data, "x"):
            data.x = data.x.to(torch.float32)
        if hasattr(data, "y"):
            # could be int64 (for classification) / float32 (for regression)
    ...
    def process(self):
        ...
        if len(self.args.dataset) == 1:
            global_dataset = load_global_dataset(
                self.global_root, scenario=self.args.scenario, dataset=self.args.dataset[0])
        else:
            global_dataset = [load_global_dataset(
                self.global_root, scenario=self.args.scenario, dataset=dataset_i) for dataset_i in self.args.dataset]
        ...
        if self.args.simulation_mode == "graph_fl_label_skew":
            ...
            self.local_data = subgraph_fl_louvain_plus(
                self.args, global_dataset)
